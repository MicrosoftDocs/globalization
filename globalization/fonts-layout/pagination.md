...
description: This article covers how text flows within text areas.
ms.date: 10/31/2023
author: jowilco
---
...
For example, English, and other languages using the Latin script, are typically written from left-to-right, from top-to-bottom of the page. Chinese may be written either horizontally (left-to-right, top-to-bottom) or vertically (top-to-bottom, right-to-left). Both directions may be present in the same document. This is called a bi-orientational script. 

## Controlling orientation in HTML using CSS

There are 3 CSS properties that allow control over the direction of text within text flows:
...
In addition to these 3 properties, you can use the `unicode-bidi` property to manage the [directionality](text-directionality.md) of text segments, including embedded sequences, isolated sequences, and overrides.

Note that you can obtain similar effects with the `rotate` and `transform` properties that you can obtain with `writing-mode`.
...
When a text is written in horizontal format, pages are read in the same order as English books, with the binding at the left and pages progressing to the right. Binding for vertical books corresponds to the reading direction on the page. For example, for Japanese using vertical text, the binding will be on the right and pages progressing to the left.
