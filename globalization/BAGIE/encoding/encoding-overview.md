...
Most applications written today handle character data primarily as Unicode, using the UTF-16 encoding. However, some legacy applications continue to use single-byte character sets and the corresponding code pages.

Windows code pages, commonly called "ANSI code pages", are code pages for which non-ASCII values (values greater than 127) represent international characters. These code pages were used natively in Windows Me, and are still available on newer versions of Windows.
